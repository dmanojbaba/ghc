...
| `cattHandler.test.ts` | `jump` routing to `/device/box/jump/:position`, `tts` routing to `/site/:value`, `cast` channel redirect (key, case-insensitive name, with device=queue, URL passthrough, unknown value), `app` command routing, `playlist` (no value→shuffle, with value→/catt cast, device set first in both cases), `stop` routing to `/device/box/stop`, `history` routing to `/device/box/history` |
