...
| `cattHandler.test.ts` | `jump` routing to `/device/box/jump/:position`, `tts` routing to `/site/:value`, `cast` channel redirect (key, case-insensitive name, with device=queue, URL passthrough, unknown value), `app` command routing |
