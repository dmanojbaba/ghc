...
- **`src/devices.ts`** — Single device (`"box"`), input aliases (`k`=Mini Kitchen, `o`=Mini Office, `b`=Mini Bedroom, `zbk`=Mini ZBK, `tv`=Google TV, `otv`=Office TV), channel list, and defaults. `orderedInputs: true` — inputs are ordered `k→o→b→zbk→tv→otv` (wrapping). `getAppKey(deviceId, input, fallback)` resolves an app key or synonym to the canonical key. `getAdjacentInput(deviceId, currentKey, delta)` returns the input key ±delta positions away (wraps around).
