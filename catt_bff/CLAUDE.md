...
`queue` table: `{ position, url, title, added_at }`. `GET /device/box/state` returns `queue` as `{ url, title }[]` ordered by position ASC, covering all pending items (including the next-to-play item). `position` is not exposed — the list is always ordered. `title` is populated (truncated to 40 chars) when items come from a YouTube playlist expansion; otherwise null. `next` is also returned as a bare URL for backwards compatibility.
