...
- **`src/googleHome.ts`** — Google Home C2C SYNC/QUERY/EXECUTE intent handlers. `selectChannel` and `relativeChannel` call `/clear` before `/cast` to ensure immediate playback rather than queuing. `mediaSeekRelative` maps positive `relativePositionMs` to `ffwd` and negative to `rewind` on `catt_server`. `mute` sends `volumemute` to `catt_server` (`volumeCanMuteAndUnmute` is `false` so Google Home does not advertise it).
