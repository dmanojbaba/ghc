...
| (integrations.test.ts AI section) | AI called for unrecognised Telegram message, device parsed from AI response, `unknown` command → "I didn't understand that", malformed JSON → graceful fallback, AI throws → graceful fallback, known command → AI not called |
